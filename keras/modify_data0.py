...
def modify(inputs, epoch):
    ...
    for input in inputs:
        ...
        modified_input = input_random_by_part(input, part_w, part_h, number_parts, np.mean(np.random.uniform(0.0, 1,samples_count)))
...
def modify_7(x_train_ori, x_train_pre, y_train_ori, pre_y_train):
    new_inputs = np.empty([0, 3, 224, 224], dtype='float32') #new inputs
    new_y = np.empty(0,dtype='int32')
    empty_labels = np.empty(4500, dtype='int32')
    ...
    for i in range(0, 102):
        ...
        #from origin
        indices = np.random.randint(len(y_train_ori_by_class[i]), size=len(y_train_ori_by_class[-1]))
        images = np.reshape(x_train_ori[y_train_ori_by_class[i][indices]],(50, 3, 224, 224))
        #from epoch - 1 round
        indices = np.random.randint(len(pre_y_train_by_class[i]), size=len(pre_y_train_by_class[-1]))
        pre_images = np.reshape(x_train_pre[pre_y_train_by_class[i][indices]],(50, 3, 224, 224))
...
def modify_8(x_train_ori, x_train_pre, y_train_ori, pre_y_train):
    new_inputs = np.empty([0, 3, 224, 224], dtype='float32') #new inputs
    new_y = np.empty(0,dtype='int32')
    empty_labels = np.empty(4500, dtype='int32')
    ...
    y_train_ori_by_class = []
    pre_y_train_by_class = []
    X_train = np.empty([0, 3, 224, 224], dtype='float32') #half of new and origin
    y_train = np.empty(0,dtype='int32')
    for i in range(0, 10):
        y_train_ori_by_class.append(np.argwhere(y_train_ori == i))
        pre_y_train_by_class.append(np.argwhere(pre_y_train == i))
        #from origin
        indices = np.random.randint(len(y_train_ori_by_class[i]), size=50)
        images = np.reshape(x_train_ori[y_train_ori_by_class[i][indices]],(50, 3, 224, 224))
        #from epoch - 1 round
        indices = np.random.randint(len(pre_y_train_by_class[i]), size=50)
        pre_images = np.reshape(x_train_pre[pre_y_train_by_class[i][indices]],(50, 3, 224, 224))

        new_images = np.mean(np.array([images,pre_images]), axis=0)

        #half of origin merge with half of new ones
        indices = np.random.randint(50/2, size=(2, 50/2))
        x = np.concatenate((np.reshape(x_train_ori[y_train_ori_by_class[i][indices[0]]], (50/2, 3, 32, 32)), new_images[indices[1]]), axis=0)
